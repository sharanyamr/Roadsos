...
import { MapIcon, Filter, Layers, Zap, ShieldAlert } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/5 p-6 rounded-[32px] border border-white/5">
        <div>
          <h3 className="text-xl font-bold flex items-center gap-3">
             <MapIcon className="text-red-500" /> Accident Hotspot Heatmap
          </h3>
          <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Real-time incident density visualization</p>
        </div>
        <div className="flex gap-2">
           <button 
             ...
             className={cn(
               ...
               activeFilter === 'density' ? "bg-red-600 text-white" : "glass text-slate-400 hover:text-white"
             )}
           >
             DENSITY MAP
           </button>
           <button 
             ...
             className={cn(
               "px-4 py-2 rounded-xl text-xs font-black transition-all",
               activeFilter === 'severity' ? "bg-red-600 text-white" : "glass text-slate-400 hover:text-white"
             )}
           >
             SEVERITY OVERLAY
           </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-[600px]">
        <div className="lg:col-span-8 glass rounded-[40px] overflow-hidden border border-white/10 relative">
          <MapContainer 
            ...
            zoom={13} 
            style={{ width: '100%', height: '100%', filter: 'invert(100%) hue-rotate(180deg) brightness(95%) contrast(90%)' }}
            zoomControl={false}
          >
            ...
            {hotspots.map((spot, i) => (
              <React.Fragment key={i}>
                <Circle 
                  ...
                  pathOptions={{ 
                    fillColor: activeFilter === 'density' ? '#ef4444' : '#f97316', 
                    color: 'transparent',
          ...
          <div className="absolute bottom-6 left-6 z-[1000] glass p-4 rounded-2xl border border-white/10 backdrop-blur-xl">
             <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Map Legend</span>
             </div>
             <div className="space-y-2">
                <div className="flex items-center gap-2">
                   <div className="w-3 h-3 rounded-full bg-red-600/60" />
                   <span className="text-[10px] font-bold text-slate-300">High Risk Zone</span>
                </div>
                <div className="flex items-center gap-2">
                   <div className="w-3 h-3 rounded-full bg-red-600/20" />
                   <span className="text-[10px] font-bold text-slate-300">Moderate Warning</span>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6 overflow-y-auto pr-2 custom-scrollbar">
           <div className="glass p-8 rounded-[32px] border border-white/5">
              <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6 px-1">Top Hazard Zones</h4>
              <div className="space-y-6">
                 {hotspots.sort((a, b) => b.intensity - a.intensity).map((spot, i) => (
                   <div key={i} className="flex flex-col gap-2">
                      <div className="flex justify-between items-center">
                         <span className="text-sm font-bold text-slate-200">{spot.label}</span>
                         <span className="text-[10px] font-black text-red-500">{(spot.intensity * 100).toFixed(0)}% RISK</span>
                      </div>
           ...
           <div className="glass p-8 rounded-[32px] border border-white/5 bg-red-600/5">
              <div className="flex items-center gap-3 mb-4">
                 ...
                 <h4 className="text-sm font-black uppercase tracking-tight">System Recommendation</h4>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed font-medium">
                 High frequency of accidents detected at <span className="text-white font-bold">Highway 101 Onramp</span> during peak hours (08:00 - 10:00). Deployment of additional responders recommended for the next 4 hours.
              </p>
